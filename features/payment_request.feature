Feature: Payment Request
  ...
  Scenario: A payment request is received from the configured remote application and I have not configured any payees in my app (default)
    ...
    And a PUT request should have been made to the remote application for the payment request: 347752, containing: the paypal response
  ...
  Scenario: I have configured a payee in my app and a payment request is received from the configured remote app to pay a different payee
    ...
    And a PUT request should have been made to the remote application for the payment request: 347752, containing: "{'payee_not_found' => true}"
  ...
  Scenario: I have configured a payee in my app and a payment request is received from the configured remote app to pay the configured payee but for an amount greater than the maximum allowed for this payee
    ...
    Then a POST request should not have been made to my paypal account
    And the payment request should be unauthorized
    And a PUT request should have been made to the remote application for the payment request: 347752, containing: "{'payee_maximum_amount_exceeded' => true}"
  ...
  Scenario: A CSRF attack triggers the process payment request task for a payment request that is not yet verified (i.e. the payment request has been created but the remote app hasn't got back to us yet)
  ...
  Scenario: A CSRF attack triggers the process payment request task for a payment request that is remotely unauthorized (i.e. the payment request was not made by the configured remotey application)
    Given an remotely unauthorized payment request exists with id: 12345
    And I am logged in
